<template>
  <div class="checkoutn_warpper PcV">
    ...
    <div class="main_warpper">
      <div class="checkoutl">
        <InsExpressWay @expressError="express=false" @express="express=true" @PickAddressError="PickAddress = false"
          @PickAddress="PickAddress = true" :checkouting="checkouting" @getAdditionlCost="showAdditionlCost"
          @getDeliveryDate="showDeliveryDate" @getRemark="showRemark" @expressChange="(value)=>{this.express = value;}"
          ...
          <div class="coupon_main">
            <div v-if="checkouting">
              <CheckboxGroup size="mini" v-model="delete1">
                <Checkbox v-for="(item,index) in coupon" :key="index" :min="0" :max="100" :label="item.Id" :disabled="item.canCheck"
            ...
            <div v-else>
              <CheckboxGroup size="mini" v-model="delete1">
                <Checkbox v-for="(item,index) in selectedCoupon" :key="index" :min="0" :max="100" :label="item.Id"
                  :disabled="true">
                  <div class="coupon_item">
                    <div class="coupon_title">{{item.Title}}</div>
                    <div class="coupon_remark" v-if="!item.IsDiscount">{{$t('Order.Full')}} {{item.MeetAmount}}
      ...
      <div class="checkoutr">
        <div class="shopCart_warpper">
          ...
          <div :style="{ height: items*144 + 'px'}" class="item_change">
            <ShoppingCartItem v-for="(item,index) in Shoppcart.Items" :key="index" :index="index" :ShopcartItem="item"
              :lock="true"></ShoppingCartItem>
          </div>
          ...
          <div class="discount">
            <p class="price_item" v-show="checkouting">
              ...
              <span style="flex-shrink:0;">
                <ElInput :placeholder="$t('CheckOut.Couponcode')" class="input-text promotion_code" v-bind:disabled="orderSure"
                  v-model="promotionCode" clearable></ElInput>
                <a href="javascript:;" class="promotion-code-btn" v-on:click="searchCoupon">{{$t('CheckOut.confirm')}}</a>
              </span>
            </p>
          ...
          <div class="price">
            <div class="price_item">
              ...
              <span>{{Shoppcart.Currency.Code}} {{parseFloat((parseFloat(cp))+ $store.state.express.DiscountPrice + AdditionlCost).toFixed(2)}}</span>
            </div>
          </div>
        </div>
    ...
    <div class="btn_warpper">
      <InsButton v-show="checkouting" style="display:inline-block;margin: 20px;" width="300px" size="middle" :nama="$t('CheckOut.BacktoShoppingCart')"
        @click="Hyperlink" />
      <InsButton style="display:inline-block;margin: 20px;" width="300px" size="middle" :nama="$t('CheckOut.Checkout')"
        @click="checkOut" :loading="COloading" />
    </div>
  </div>
</template>
...
<script lang="ts">
  ...
  @Component({
    components: {
      ShoppingCartItem: () => import(/* webpackChunkName: "checkout" */
        '@/components/business/pc/account/InsShoppingcartItem.vue'),
  ...
  export default class InsCheckoutN extends Vue {
    ...
    created() {
      ...
      Promise.all([this.$store.state.shopCart, this.$store.state.address, pm]).then((result) => {
        ...
        this.$HiddenLayer();
        this.$Api.member.getActiveCoupon({
          Page: 1,
          PageSize: 10
        }).then((result) => {
    ...
    checkOut() {
      if (this.$store.state.DeliveryType === 'D' && !this.express) {
        this.$Confirm(this.$t('Message.Message'), this.$t('CheckOut.expressError'));
        return;
      }
      if (this.$store.state.DeliveryType === 'P' && !this.PickAddress) {
        this.$Confirm(this.$t('Message.Message'), this.$t('CheckOut.PickAddressError'));
        return;
      ...
      if (this.$store.state.DeliveryType === 'D' && !this.PickAddress && this.$store.state.pickUpExpress) {
        this.$Confirm(this.$t('Message.Message'), this.$t('CheckOut.PickAddressError'));
        return;
      }
    ...
    pay() {
      ...
      if (this.$store.state.DeliveryType === 'D') {
 order = new CreateOrder(this.$store.state.pickUpExpress ? -1 : this
        .$store.state.selectAddress.DeliveryId, this.$store.state.express.Id, this.payment.Id, 'D', '', '', this.$store
        .state.DPickPhone, this.$store.state.DPickName, this.promotionCode, this.$store.state.expressPoint.Id, this
        .delete1, this.TimeRangeId, this.DeliveryDate, this.Remark);
} else if (this.$store.state.DeliveryType === 'P') {
 order = new CreateOrder(this.$store.state.pickupAddress.Id, '',
        this.payment.Id, 'P', this.$store.state.pickupAddress.PickupDate, this.$store.state.pickupAddress.PickupTime,
      ...
      if ((!order.PickupName && !profile.FirstName) && (!order.Mobile && !profile.PickupPhone)) { // 若無姓名與電話， 則updateprofile
        this.$Confirm(
          ...
          this.$t('CheckOut.Use') + ' ' + order.PickupName + ' ' + this.$t('CheckOut.As') + ' ' + this.$t(
            'CheckOut.Name') + '</br>' +
          this.$t('CheckOut.Use') + ' ' + order.PickupPhone + ' ' + this.$t('CheckOut.As') + ' ' + this.$t(
            'CheckOut.Phone'),
          () => {
    ...
    get cp() {
      return this.discount.Id === '-1'
        ? this.currentPrice
        : (this.discount.IsDiscount ? ((this.currentPrice) * (1 - this.discount.Favorable / 100)).toFixed(2) : (this.currentPrice) -
          this.discount.Favorable);
    }
    get currentCode() {
      return this.$store.state.currency.Code;
    }
    searchCoupon() {
      if (this.totalAmount !== this.currentPrice) {
        this.$Confirm(this.$t('Message.Message'), this.$t('CheckOut.CandP'));
        return;
      }
